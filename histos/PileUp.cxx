...
void PoleZeroHistos(TRuntimeObjects& obj, TCagraHit& core_hit, string local_dirname = "") {
  ...
  obj.FillHistogram(local_dirname, stream.str(),1000,0,10000,core_hit.Charge(),1000,0,3000,prerise_base);

  stream.str("");
  stream << "E_BL_scale" << boardid << "_" << chan;
  obj.FillHistogram(local_dirname, stream.str(),1000,0,10000,core_hit.Charge()-(1.0/-11.21)*prerise_base,1000,0,3000,prerise_base);

  stream.str("");
  stream << "Ge_PZ_" << boardid << "_" << chan;
  obj.FillHistogram(local_dirname, stream.str(),5000,0,10000,core_hit.GetCorrectedEnergy(0));
  stream.str("");
  stream << "Ge_PZ_AsymBL_" << boardid << "_" << chan;
  obj.FillHistogram(local_dirname, stream.str(),5000,0,10000,core_hit.GetCorrectedEnergy(core_hit.GetBaseSample()));

  stream.str("");
  stream << "E_cor_BL" << boardid << "_" << chan;
  obj.FillHistogram(local_dirname, stream.str(),1000,0,10000,core_hit.GetCorrectedEnergy(core_hit.GetBaseSample()),1000,0,3000,prerise_base);
}
...
void MakeCAGRAHistograms(TRuntimeObjects& obj, TCagraHit& cagra) {
  ...
  for (auto& core_hit : cagra) {
    ...
    obj.FillHistogram("CAGRA_Raw", stream.str(),5000,0,10000,core_hit.Charge());
    for (auto& segment : core_hit) {
      stream.str("");
      stream << "Ge_" << boardid << "_" << segment.GetChannel();
      obj.FillHistogram("CAGRA_Raw",stream.str(),5000,0,10000,segment.Charge());
    }

    stream.str("");
    stream << "Ge_" << boardid << "_" << chan;
    obj.FillHistogram("CAGRA_Calibrated",stream.str(),5000,0,10000,core_hit.GetEnergy());
    for (auto& segment : core_hit) {
      stream.str("");
      stream << "Ge_" << boardid << "_" << segment.GetChannel();
      obj.FillHistogram("CAGRA_Calibrated",stream.str(),5000,0,10000,segment.GetEnergy());
    }

    // skip polezero for LaBr3 system
    if ( core_hit.GetSystem() != 'L') {
      PoleZeroHistos(obj,core_hit);
    }
    ...
    if (false && core_hit.GetDetnum() <= 104) {
      ...
      obj.FillHistogram("CAGRA_Raw",stream.str(),5000,0,10000,labr_E);
    }
  ...
  for (auto& hit : gr) {
    ...
    for (auto& core_hit : cagra) {
      ...
      if ( (tdiff > 235) && (tdiff < 245) ){
        ...
        obj.FillHistogram("COIN_Raw",stream.str(),5000,0,10000,core_hit.Charge());
        stream.str("");
        stream << "Ge_" << boardid << "_" <<channel;
        obj.FillHistogram("COIN_Calibrated",stream.str(),5000,0,10000,core_hit.GetEnergy());
        for (auto& segment : core_hit) {
          stream.str("");
          stream << "Ge_" << boardid << "_" << segment.GetChannel();
          obj.FillHistogram("COIN_Calibrated",stream.str(),5000,0,10000,segment.GetEnergy());
        }
